...
@Component({
  ...
  template: `
    <div class="doc-content">
      ...
      <p>
        Una biblioteca de notificaciones toast para Angular con una API completamente basada en
        Promises. Construida sobre <code>@angular/cdk/overlay</code> — sin dependencias externas
        adicionales.
      </p>
      <p>
        Cada toast es un objeto awaitable (<code>JastToastRef</code>): podés esperar a que cierre,
        cancelarlo programáticamente, o encadenar acciones. El servicio maneja posicionamiento,
        apilado, animaciones spring physics y limpieza del DOM de forma automática.
      </p>
      <p>
        Como es <code>providedIn: 'root'</code>, no necesitás configurarlo en ningún módulo ni en
        <code>app.config.ts</code>. Lo inyectás y listo.
      </p>
      ...
      <div class="code-editor">
        ...
        <div class="editor-body">
          <button
            class="copy-btn"
            [class.copied]="copiedJast"
            (click)="copyCommand('npm install jast-notification', 'jast')"
          >
            @if (copiedJast) {
              <svg
                viewBox="0 0 24 24"
      ...
      <p>
        Si todavía no tenés <code>&#64;angular/cdk</code> en tu proyecto, es necesario instalarlo
        como dependencia de pares (peer dependency):
      </p>
      ...
      <div class="code-editor">
        ...
        <div class="editor-body">
          <button
            class="copy-btn"
            [class.copied]="copiedCdk"
            (click)="copyCommand('npm install @angular/cdk', 'cdk')"
          >
            @if (copiedCdk) {
              <svg
                viewBox="0 0 24 24"
      ...
      <p>
        Inyectá <code>JastNotificationService</code> en cualquier componente standalone y llamá a
        uno de sus métodos. No hace falta importar nada en el template.
      </p>
      ...
      <p>
        El servicio expone métodos abreviados para los tipos más comunes y un método genérico
        <code>show()</code> que acepta configuración completa.
      </p>
      <div class="code-block">
        <pre><code>{{ methodsCode }}</code></pre>
      </div>
      <p class="note">
        <strong>Nota:</strong> <code>success()</code> y <code>error()</code> son atajos que omiten
        el campo <code>type</code>. Para <code>'warning'</code> e <code>'info'</code> usá
        <code>show()</code> con el tipo explícito.
      </p>
    </div>
  `,
  styles: [
